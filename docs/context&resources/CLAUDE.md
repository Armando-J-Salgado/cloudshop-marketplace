...
NOTA: ES MANDATORIO REVISAR CADA UNO DE ESTOS ARCHIVOS CADA QUE SE ESTÉ REALIZANDO UN PLAN:
...
- El flujo de trabajo asistido por IA para este proyecto es el siguiente:
  ...
  4. Convención de documentación: dentro de `docs/`, cada tarea tiene su propia carpeta nombrada según la tarea (por ejemplo `docs/gh-actions-iam/`), con un `spec.md` (el spec plan) y un `walkthrough.md` (el informe de cierre) dentro. Reemplaza al esquema anterior de `docs/spec-plans/` y `docs/walkthroughs/` para las tareas nuevas.
...
- Para validar cambios con todo el equipo, se hace uso de GitHub Actions (`.github/workflows/terraform.yml`) con el usuario IAM `github-actions` (mínimo privilegio, creado vía Terraform en `infraestructure/cicd/`) para validar y planificar el despliegue de la versión más reciente del proyecto. El despliegue automático (`terraform apply` desde el pipeline) permanece deshabilitado hasta que exista backend remoto de Terraform. Ver [docs/gh-actions-iam/spec.md](../gh-actions-iam/spec.md) y [docs/gh-actions-iam/walkthrough.md](../gh-actions-iam/walkthrough.md) para el detalle completo y los próximos pasos pendientes.
